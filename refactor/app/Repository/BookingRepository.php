...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($jobs) {
            ...
            $noramlJobs = collect($noramlJobs)->each(
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            if (!empty($data['from_language_id'])) {
                return $this->failResponse('from_language_id');
            }

            if ($data['immediate'] == 'no') {
                if (!empty($data['due_date'])) {
                    return $this->failResponse('due_date');
                }
                if (!empty($data['due_time'])) {
                    return $this->failResponse('due_time');
                }
                if (empty($data['customer_phone_type'])) {
                    return $this->failResponse('customer_phone_type');
                }
            }

            if (!empty($data['duration'])) {
                return $this->failResponse('duration');
            }

            if (!empty($data['customer_physical_type'])) {
    ...
    public function jobEnd($post_data = array())
    {
        ...
        Event::fire(
            new SessionEnded(
                $job,
                ($post_data['userid'] == $job->user_id) ? $tr->user_id : $job->user_id
            )
        );
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        foreach ($job_ids as $key => $value)     // checking translator town
        {
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $msg_contents = '';
        if ($data['immediate'] == 'no') {
            $msg_contents = 'Ny bokning för ' .
                $data['language'] . 'tolk ' .
                $data['duration'] . 'min ' .
                $data['due'];
        } else {
            $msg_contents = 'Ny akutbokning för ' .
                $data['language'] . 'tolk ' .
                $data['duration'] . 'min';
        }
        $msg_text = array(
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        foreach ($translators as $translator) {
            ...
            Log::info(
                `Send SMS to {$translator->email} ({$translator->mobile}), status: ` . print_r($status, true)
            );
        }
    ...
    public function sendSessionStartRemindNotification($user, $job, $language, $due, $duration)
    {
        ...
        if ($job->customer_physical_type == 'yes')
            $msg_text = array(
                "en" => 'Detta är en påminnelse om att du har en ' . $language .
                    'tolkning (på plats i ' . $job->town . ') kl ' .
                    $due_explode[1] . ' på ' . $due_explode[0] .
                    ' som vara i ' . $duration . ' min. Lycka till och kom ihåg att ge feedback efter utförd tolkning!'
            );
        else
            $msg_text = array(
                "en" => 'Detta är en påminnelse om att du har en ' . $language .
                    'tolkning (telefon) kl ' . $due_explode[1] . ' på ' .
                    $due_explode[0] . ' som vara i ' .
    ...
    public function sendExpiredNotification($job, $user)
    {
        ...
        $msg_text = array(
            ...
            'Tyvärr har ingen tolk accepterat er bokning: (' .
                $language . ', ' . $job->duration . 'min, ' . $job->due .
    ...
    public function cancelJobAjax($data, $user)
    {
        ...
        /*@todo
            ...
            if cancelation is within 24 hours - translator will be informed AND the customer will get an
            addition to his number of bookings - so we will charge of it if the cancelation is within 24 hours
        ...
        */
        ...
        if ($cuser->is('customer')) {
            ...
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                $response['status'] = 'fail';
                $response['message'] =
                    'Du kan inte avboka en bokning som sker inom 24 timmar genom DigitalTolk.
    ...
    private function failResponse($field_name)
    {
        return [
            'status' => 'fail',
